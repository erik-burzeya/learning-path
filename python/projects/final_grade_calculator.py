...
def main():
    ...
    while True:  # While loop: making it possible to stop or repeat the calculation
        ...
        score = int(input(
            "What is your score for block I.? Type your score "
            "(Calculated with E=P/U * 40): "))
        if score < 200:
        ...
        print()
        print("Your marks may vary slightly. The following is a range "
              "within which your actual marks are likely to fall:")
        ...
        for i in range(len(subjects)):
            ...
            print(
                f"{subjects[i]:15}: {min_points1:02}-{max_points1:02} Punkte")

        print()

        # POINT RANGE FOR BLOCK II.
        print(
            f"Your total amount of points for block II. "
            "could lie between {total_min*5} and {total_max*5} Points.")

        # TOTAL AMOUNT OF POINTS
        print("All points that have been archieved in the final exams "
              "are multiplied by 5 and added to the points of block I.")
        print("The points of block I. and block II. make up the "
              "total amount of points.")

        print()

        print(
            f"Your total amount of points could lie between {(score + total_min*5)} "
            f"and {(score + total_max*5)} points")
        restart = input(
            "Do you want to start another calculation? (yes/no): ").lower()
    ...
    while True:
        user_choice = input(
            "Do you want to calculate your grade? (yes/no): ").lower()
